...
public class BillGT {
    ...
    public void printBill(String sdt, String manv) {
        this.sdt = sdt;
        this.manv = manv;
        bHeight = Double.valueOf(GioHangGT.listGT.size());
...
//            int headerRectHeight=40;
                    ...
                    g2d.drawString("-------------------------------------", 12, y);
                    y += yShift;
                    g2d.drawString("           HÓA ĐƠN GÓI TẬP          ", 12, y);
                    y += yShift;
                    ...
                    g2d.drawString(" Tên gói tập               Thành tiền   ", 10, y);
                    y += yShift;
                    g2d.drawString("--------------------------------------", 10, y);
                    y += headerRectHeight;
                    
                    for (GoiTap goiTap : GioHangGT.listGT) {
                        g2d.drawString(" " + goiTap.getTenGoiTap() + "                            ", 10, y);
                        y += yShift;
                        g2d.drawString(" Thời lượng:" + goiTap.getThoiLuong()+" ngày", 10, y);
                        g2d.drawString(df.format(goiTap.getGia()), 160, y);
                        y += yShift;
                    }
                    g2d.drawString("-------------------------------------", 10, y);
                    y += yShift;
                    g2d.drawString(" Tổng tiền:                " + df.format(GioHangGT.tongTienGH()) + "   ", 10, y);
                    y += yShift;
                    g2d.drawString("-------------------------------------", 10, y);
                    y += yShift;
                    g2d.drawString(" Giảm giá:                  " + GioHangGT.giamGia*100 + "%   ", 10, y);
                    y += yShift;
                    g2d.drawString("-------------------------------------", 10, y);
                    y += yShift;
                    g2d.drawString(" Thanh toán:              " + df.format(GioHangGT.tienThanhToan()) + "   ", 10, y);
                    y += yShift;
